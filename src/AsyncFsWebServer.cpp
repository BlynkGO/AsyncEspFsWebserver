...
bool AsyncFsWebServer::init(AwsEventHandler wsHandle) {
    ...
    //on("/favicon.ico", ASYNC_HTTP_GET, std::bind(&AsyncFsWebServer::sendOK, this, _1));
    on("/connect", ASYNC_HTTP_POST, std::bind(&AsyncFsWebServer::doWifiConnection, this, _1));
    on("/scan", ASYNC_HTTP_GET, std::bind(&AsyncFsWebServer::handleScanNetworks, this, _1));
    on("/wifistatus", ASYNC_HTTP_GET, std::bind(&AsyncFsWebServer::getStatus, this, _1));
    on("/clear_config", ASYNC_HTTP_GET, std::bind(&AsyncFsWebServer::clearConfig, this, _1));
    on("/setup", ASYNC_HTTP_GET, std::bind(&AsyncFsWebServer::handleSetup, this, _1));
    on("*", ASYNC_HTTP_HEAD, std::bind(&AsyncFsWebServer::handleFileName, this, _1));

    on("/upload", ASYNC_HTTP_POST,
        std::bind(&AsyncFsWebServer::sendOK, this, _1),
        std::bind(&AsyncFsWebServer::handleUpload, this, _1, _2, _3, _4, _5, _6)
    );

    on("/update", ASYNC_HTTP_POST,
        std::bind(&AsyncFsWebServer::update_second, this, _1),
        std::bind(&AsyncFsWebServer::update_first, this, _1, _2, _3, _4, _5, _6)
    );

    on("/reset", ASYNC_HTTP_GET, [](AsyncWebServerRequest *request) {
        request->send(200, "text/plain", WiFi.localIP().toString());
        delay(500);
        ESP.restart();
    });

    on("/wifi", ASYNC_HTTP_GET, [](AsyncWebServerRequest *request) {
        String reply = "{\"ssid\":\"";
        reply += WiFi.SSID();
        reply += "\", \"rssi\":";
        reply += WiFi.RSSI();
    ...
    on("/get_config", ASYNC_HTTP_GET, [](AsyncWebServerRequest *request) {
...
void AsyncFsWebServer::enableFsCodeEditor() {
    ...
    using namespace std::placeholders;
    on("/status", ASYNC_HTTP_GET, std::bind(&AsyncFsWebServer::handleFsStatus, this, _1));
    on("/list", ASYNC_HTTP_GET, std::bind(&AsyncFsWebServer::handleFileList, this, _1));
    on("/edit", ASYNC_HTTP_PUT, std::bind(&AsyncFsWebServer::handleFileCreate, this, _1));
    on("/edit", ASYNC_HTTP_DELETE, std::bind(&AsyncFsWebServer::handleFileDelete, this, _1));
    on("/edit", ASYNC_HTTP_GET, std::bind(&AsyncFsWebServer::handleFileEdit, this, _1));
    on("/edit", ASYNC_HTTP_POST,
        std::bind(&AsyncFsWebServer::sendOK, this, _1),
        std::bind(&AsyncFsWebServer::handleUpload, this, _1, _2, _3, _4, _5, _6)
    );
#endif
