...
const __FlashStringHelper *AsyncWebServerRequest::methodToString() const {
  if(_method == ASYNC_HTTP_ANY) return F("ANY");
  else if(_method & ASYNC_HTTP_GET) return F("GET");
  else if(_method & ASYNC_HTTP_POST) return F("POST");
  else if(_method & ASYNC_HTTP_DELETE) return F("DELETE");
  else if(_method & ASYNC_HTTP_PUT) return F("PUT");
  else if(_method & ASYNC_HTTP_PATCH) return F("PATCH");
  else if(_method & ASYNC_HTTP_HEAD) return F("HEAD");
  else if(_method & ASYNC_HTTP_OPTIONS) return F("OPTIONS");
  return F("UNKNOWN");
}
